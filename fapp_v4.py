...
# run_score "couleur" : percentile global sur tous les runs (N*60)
...
tab_method, tab_run, tab_foil, tab_overview, tab_decision = st.tabs(
    ["Features & Méthodo", "Run detail", "Foil detail", "Overview", "Decision"]
)
...
with tab_method:
    ...
    st.caption(
        "Document décisionnel basé sur les simulations (6 foils × 60 réglages) — "
        "normalisation par percentile global (0–1)."
    )

    # -----------------------------
    # A) Objectif & périmètre
    # -----------------------------
    st.markdown(
        """
**Objectif**  
Sélectionner le meilleur candidat de foil pour le prochain bateau sur la base d’un plan de simulation commun.

**Périmètre**  
- Chaque foil est évalué sur les mêmes 60 réglages : `Travel`, `KeelCant`, `FoilRake`.  
- Chaque run dure **120 secondes** (échantillonnage **20 Hz**).  
- La décision cherche un compromis **performance utile + exploitabilité** : stabilité, absence de dégradation, robustesse aux réglages.
"""
    )
...
- **foil_score (foil-level)** = mix :
  - performance moyenne (mean_run_score)  
  - robustesse inter-runs (plateau + dispersion)
"""
    )

    # with st.expander("Voir les poids utilisés (lecture seule)", expanded=False):
    #     st.write("Run-level (blocs 1–3) :", run_block_weights)
    #     st.write("Mix foil-level :", foil_mix_weights)
    #     st.write("Poids bloc 4 :", foil_block_weights)

    st.divider()

    # -----------------------------
    # C) Recommandation (Top 3)
    # -----------------------------
    st.markdown("### Recommandation (Top 3)")
    top3 = scored_foils.head(3).copy()

    c1, c2, c3 = st.columns(3)
    for col, (_, row) in zip([c1, c2, c3], top3.iterrows()):
        with col:
            st.metric(
                label=f"Foil {row['foil_id']}",
                value=f"{row['foil_score']:.2f}",
                help="Score final (performance + robustesse)"
            )
            st.caption(
                f"mean_run_score={row.get('mean_run_score', np.nan):.2f} • "
                f"Plateau={row.get('Plateau_pct', np.nan):.1f}% • "
                f"Disp(IQR)={row.get('InterRun_VMG_IQR', np.nan):.2f}"
            )

    st.markdown(
        """
**Lecture**  
- **Choix recommandé** : Foil #1 (meilleur compromis performance utile + robustesse).  
- **Alternative performance max** : Foil #2 (si vous privilégiez le plafond au détriment de la tolérance réglage).  
- **Alternative robuste** : Foil #3 (si vous privilégiez stabilité/robustesse).
"""
    )
    ...
    # -----------------------------
    # D) Classement synthétique (Top 10)
    # -----------------------------
    st.markdown("### Classement synthétique")

    cols_show = ["foil_id", "foil_score", "mean_run_score", "Plateau_pct", "InterRun_VMG_IQR"]
    cols_show = [c for c in cols_show if c in scored_foils.columns]
    st.dataframe(scored_foils[cols_show].head(10), use_container_width=True, hide_index=True)

    st.divider()

    # -----------------------------
    # E) Pareto performance vs robustesse (résumé)
    # -----------------------------
    st.markdown("### Pareto : performance vs robustesse")
    st.caption("X = mean_run_score (↑) • Y = InterRun_VMG_IQR (↓)")

    tmp = scored_foils.copy()
    if "mean_run_score" in tmp.columns and "InterRun_VMG_IQR" in tmp.columns:
        tmp["pareto"] = pareto_frontier_2d(tmp, "mean_run_score", "InterRun_VMG_IQR", x_high=True, y_high=False)
        ...
        chart = (
            ...
            .encode(
                x=alt.X("mean_run_score:Q", title="Mean run_score (↑)"),
                y=alt.Y("InterRun_VMG_IQR:Q", title="Dispersion inter-run (IQR, ↓)"),
                color=alt.Color("pareto:N", title="Pareto"),
                tooltip=["foil_id", "foil_score", "mean_run_score", "Plateau_pct", "InterRun_VMG_IQR"],
            )
            .interactive()
        ...
        st.info("Colonnes mean_run_score / InterRun_VMG_IQR absentes : Pareto indisponible.")

    st.divider()

    # -----------------------------
    # F) Forces / faiblesses (auto, simple et défendable)
    # -----------------------------
    st.markdown("### Forces & points de vigilance (Top 3)")

    # Heuristiques simples basées sur quantiles pour générer des bullets
    # (évite d'inventer — on se base uniquement sur scored_foils)
    q = {}
    for c in ["foil_score", "mean_run_score", "Plateau_pct", "InterRun_VMG_IQR"]:
        if c in scored_foils.columns:
            s = pd.to_numeric(scored_foils[c], errors="coerce")
            q[c] = {
                "p25": float(s.quantile(0.25)),
                "p75": float(s.quantile(0.75)),
            }

    def bullets_for_foil(row: pd.Series) -> tuple[list[str], list[str]]:
        strengths, risks = [], []
        # mean_run_score
        if "mean_run_score" in row and "mean_run_score" in q:
            if row["mean_run_score"] >= q["mean_run_score"]["p75"]:
                strengths.append("Performance exploitable élevée (mean_run_score dans le top quartile).")
            elif row["mean_run_score"] <= q["mean_run_score"]["p25"]:
                risks.append("Performance exploitable plus faible (mean_run_score dans le bas quartile).")
        # plateau
        if "Plateau_pct" in row and "Plateau_pct" in q:
            if row["Plateau_pct"] >= q["Plateau_pct"]["p75"]:
                strengths.append("Large sweet spot (Plateau% élevé) → tolérance aux réglages.")
            elif row["Plateau_pct"] <= q["Plateau_pct"]["p25"]:
                risks.append("Sweet spot étroit (Plateau% faible) → réglage plus pointu.")
        # dispersion
        if "InterRun_VMG_IQR" in row and "InterRun_VMG_IQR" in q:
            if row["InterRun_VMG_IQR"] <= q["InterRun_VMG_IQR"]["p25"]:
                strengths.append("Faible sensibilité aux réglages (dispersion inter-run faible).")
            elif row["InterRun_VMG_IQR"] >= q["InterRun_VMG_IQR"]["p75"]:
                risks.append("Sensibilité aux réglages (dispersion inter-run élevée).")
        return strengths, risks

    for _, r in top3.iterrows():
        strengths, risks = bullets_for_foil(r)
        st.markdown(f"**Foil {r['foil_id']}**")
        if strengths:
            st.markdown("- ✅ " + "\n- ✅ ".join(strengths))
        else:
            st.markdown("- ✅ Profil global cohérent (pas d’avantage très marqué sur les indicateurs principaux).")
        if risks:
            st.markdown("- ⚠️ " + "\n- ⚠️ ".join(risks))
        else:
            st.markdown("- ⚠️ Aucun signal de risque dominant sur les indicateurs agrégés.")
        st.write("")

    st.divider()

    # -----------------------------
    # G) Plan de validation
    # -----------------------------
    st.markdown(
        """
### Plan de validation recommandé
1) Vérifier 2–3 réglages “représentatifs” + 1–2 réglages “difficiles” (zones où les foils se séparent).  
2) Contrôler visuellement (Run detail) : oscillations (heel/trim/speed), drawdown, dérive leeway.  
3) Confirmer que le foil recommandé conserve ses avantages sur des réglages voisins (tolérance).
"""
    )

    # -----------------------------
    # H) Export rapide (texte)
    # -----------------------------
    summary_text = f"""Synthèse & recommandation

Objectif
Sélectionner le meilleur candidat de foil sur la base d’un plan de simulation commun (N foils × 60 réglages). Runs de 120s à 20Hz. Décision robuste : performance utile + stabilité + absence de dégradation + robustesse réglages.

Méthode
- Normalisation par percentile global (0..1) sur l’ensemble des runs (N×60).
- run_score (run-level) = 3 blocs : performance utile (VMG) + stabilité (variabilité robuste) + dégradation (dérive/collapses).
- foil_score (foil-level) = mix : mean_run_score + robustesse inter-run (plateau + dispersion).

Top 3 (score final)
{top3[['foil_id','foil_score','mean_run_score','Plateau_pct','InterRun_VMG_IQR']].to_string(index=False)}

Recommandation
- Choix recommandé : {top3.iloc[0]['foil_id'] if len(top3)>0 else 'N/A'}
- Alternative performance max : {top3.iloc[1]['foil_id'] if len(top3)>1 else 'N/A'}
- Alternative robuste : {top3.iloc[2]['foil_id'] if len(top3)>2 else 'N/A'}

Plan de validation
1) Vérifier réglages représentatifs et difficiles.
2) Contrôle time-series : oscillations, drawdown, dérive leeway.
3) Confirmer tolérance aux réglages sur des voisins.
"""
    # st.download_button(
    #     "Télécharger la synthèse (TXT)",
    #     data=summary_text.encode("utf-8"),
    #     file_name="synthese_foil_selection.txt",
    #     mime="text/plain",
    # )
...
with tab_foil:
    ...
    show_cols = ["KeelCant", "Travel", "FoilRake", "script_id", "run_score", "VMG_mean", "VMG_95p", "IQR_Speed", "MaxDrawdown_Speed_pct", "Delta_Speed_LastFirst", "Slope_AbsLeeway"]
...
with tab_run:
    ...
    # st.dataframe(styled, use_container_width=True, hide_index=True)
        ...
        # -----------------------------
    # Tableau comparatif (scores NORMALISÉS GLOBAUX 0–1)
    # -> on utilise les colonnes __score calculées sur TOUS les runs (N*60)
    # -----------------------------
    metric_specs = {
    "run_score": "high",
    "VMG_mean": "high",
    ...
    # On construit la liste des colonnes de score (0..1) à afficher
    score_cols = []
    rename_map = {}

    for m in available_metrics:
        if m == "run_score":
            # -> on utilise run_score__score (percentile global), pas run_score brut
            if "run_score__score" in subset.columns:
                score_cols.append("run_score__score")
                rename_map["run_score__score"] = "run_score"
        else:
            sc = f"{m}__score"
    ...
    score_df = subset[base_cols + score_cols].copy().rename(columns=rename_map)

    # Colonnes à colorer (toutes sont maintenant dans [0,1])
    heat_cols = [c for c in available_metrics if c in score_df.columns]

    # Tri (je te conseille de trier sur run_score brut, pas sur le percentile)
    score_df = score_df.merge(
        subset[["foil_id", "run_score"]], on="foil_id", how="left", suffixes=("", "_raw")
    ).sort_values("run_score", ascending=False).drop(columns=["run_score_raw"]).reset_index(drop=True)
